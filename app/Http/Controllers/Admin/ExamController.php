...
class ExamController extends Controller
{
    ...
    public function results()
    {
        ...
        $results = ExamAttempt::with([
                'user:id,name,email,phone,school_name', // Select specific user fields
                'user.field:id,name', // User's field/specialization
                'exam:id,name,total_grade,pass_grade' // Exam details
            ])
            ->where('status', 'completed') // Only show completed attempts
            ->latest('submitted_at') // Order by latest submissions
            ->paginate(20); // Paginate results

        // Alternative: Get results with additional statistics
        $resultsWithStats = ExamAttempt::with([
                'user:id,name,email,phone,school_name',
                'user.field:id,name',
                'exam:id,name,total_grade,pass_grade'
            ])
            ->selectRaw('
                exam_attempts.*,
                CASE 
                    WHEN score >= (SELECT pass_grade FROM exams WHERE id = exam_attempts.exam_id) 
                    THEN "passed" 
                    ELSE "failed" 
    ...
    // Alternative method with filtering options
    public function resultsFiltered(Request $request)
    {
        ...
        if ($request->filled('result_status')) {
            if ($request->result_status === 'passed') {
                $query->whereRaw('score >= (SELECT pass_grade FROM exams WHERE id = exam_attempts.exam_id)');
        ...
        $results = $query->latest('submitted_at')->paginate(20);
        $exams = Exam::select('id', 'name')->get(); // For filter dropdown

        return view('admin.exams.results', compact('results', 'exams'));
    }
